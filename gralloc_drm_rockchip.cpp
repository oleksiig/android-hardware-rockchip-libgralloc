        ...
        struct gralloc_drm_handle_t *handle)
{
    ...
    ALOGV("%s: enter, w : %d, h : %d, format : 0x%x, usage : 0x%x.", __func__, w, h, format, usage);
...
#define MAGIC_USAGE_FOR_AFBC_LAYER     (0x88)
...
#if RK_DRM_GRALLOC
        ALOGD_IF(RK_DRM_GRALLOC_DEBUG,"drmPrimeFDToHandle: Got handle %d for fd %d\n", gem_handle, handle->prime_fd);
#else
        ALOGD("drmPrimeFDToHandle: Got handle %d for fd %d\n", gem_handle, handle->prime_fd);
#endif
    ...
    ALOGD_IF(RK_DRM_GRALLOC_DEBUG, "leave, w : %d, h : %d, format : 0x%x,internal_format : 0x%" PRIu64 ", usage : 0x%x. size=%d,pixel_stride=%d,byte_stride=%d",
